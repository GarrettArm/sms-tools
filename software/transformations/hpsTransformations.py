...
def hpsMorph(hfreq1, hmag1, stocEnv1, hfreq2, hmag2, stocEnv2, hfreqIntp, hmagIntp, stocIntp):
    ...
    for l in range(L1):                                      # generate morphed frames
        ...
        harmonics = np.intersect1d(np.array(np.nonzero(hfreq1[l, :]), dtype=np.int)[0], np.array(
            np.nonzero(hfreq2[round(L2 * l / float(L1)), :]), dtype=np.int)[0])
        # interpolate the frequencies of the existing harmonics
        yhfreq[l, harmonics] = (1 - hfreqIndexes[l]) * hfreq1[l, harmonics] + \
            hfreqIndexes[l] * hfreq2[round(L2 * l / float(L1)), harmonics]
        # interpolate the magnitudes of the existing harmonics
        yhmag[l, harmonics] = (1 - hmagIndexes[l]) * hmag1[l, harmonics] + \
            hmagIndexes[l] * hmag2[round(L2 * l / float(L1)), harmonics]
