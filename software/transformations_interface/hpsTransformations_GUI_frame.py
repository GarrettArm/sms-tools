...
class HpsTransformations_frame:
    ...
    def initUI(self):
        ...
        preview = Button(self.parent, text=">", command=lambda: UF.wavplay(
            self.filelocation.get()), bg="gray30", fg="white")
        ...
        self.output = Button(self.parent, text=">", command=lambda: UF.wavplay('output_sounds/' +
                                                                               os.path.basename(self.filelocation.get())[:-4] + '_hpsModel.wav'), bg="gray30", fg="white")
        ...
        self.compute = Button(self.parent, text="Apply Transformation",
                              command=self.transformation_synthesis, bg="dark green", fg="white")
        self.compute.grid(row=13, column=0, padx=5, pady=(10, 15), sticky=W)

        # BUTTON TO PLAY TRANSFORMATION SYNTHESIS OUTPUT
        self.transf_output = Button(self.parent, text=">", command=lambda: UF.wavplay(
            'output_sounds/' + os.path.basename(self.filelocation.get())[:-4] + '_hpsModelTransformation.wav'), bg="gray30", fg="white")
    ...
    def analysis(self):
        ...
        try:
            ...
            self.inputFile, self.fs, self.hfreq, self.hmag, self.mYst = hT.analysis(
                inputFile, window, M, N, t, minSineDur, nH, minf0, maxf0, f0et, harmDevSlope, stocf)
    ...
    def transformation_synthesis(self):
        ...
        try:
            ...
            hT.transformation_synthesis(inputFile, fs, hfreq, hmag, mYst, freqScaling,
                                        freqStretching, timbrePreservation, timeScaling)
